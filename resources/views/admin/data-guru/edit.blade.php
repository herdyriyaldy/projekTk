...
<div class="container-fluid">
    <div class="row">
        <div class="col-md-12">
            <div class="card">
                <div class="card-body">
                    <div class="card-title">{{ __('Edit Data Guru') }}</div>

                    <form method="post" action="{{ route('data-guru.update',$guru->id) }}">
                        @csrf
                        @method('put')
                        ...
                        <div class="input-group mb-3">
                            ...
                            <select name="matapelajaran" class="custom-select @error('matapelajaran') is-invalid @enderror" {{ count($matapelajaran) == 0 ? 'disabled' : '' }}>
                                ...
                                <option value="{{ $value->id }}" {{ $guru->id_matapelajaran == $value->id ? 'selected' : '' }}>{{ $value->matapelajaran }}</option>
                                @endforeach
                                @endif
                            </select>
                        </div>
                        ...
                        <div class="input-group mb-3">
                            ...
                            <select name="kelas" class="custom-select @error('kelas') is-invalid @enderror" {{ count($kelas) == 0 ? 'disabled' : '' }}>
                                ...
                                <option value="{{ $value->id }}" {{ $guru->id_kelas == $value->id ? 'selected' : '' }}>{{ $value->nama_kelas }}</option>
                                @endforeach
                                @endif
                            </select>
                        </div>
                        ...
                        <div class="form-group">
                            <label>Alamat</label>
                            <textarea class="form-control @error('alamat') is-invalid @enderror" rows="5" name="alamat">{{ $guru->alamat }}</textarea>
                            <span class="text-danger">@error('alamat') {{ $message }} @enderror</span>
                        </div>
                        <div class="form-group">
                            <label>Nomor Telepon</label>
                            <input type="text" class="form-control @error('nomor_telp') is-invalid @enderror" name="nomor_telp" value="{{ $guru->nomor_telp }}">
